...
bool ImGui::SetDragDropPayload(const char* type, const void* data, size_t data_size, ImGuiCond cond)
{
	...
	IM_ASSERT(type != NULL);
	IM_ASSERT(strlen(type) < IM_ARRAYSIZE(payload.NetworkTag) && "Payload type can be at most 32 characters long");
	...
	{
		// Copy payload
		ImStrncpy(payload.NetworkTag, type, IM_ARRAYSIZE(payload.NetworkTag));
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
	...
	if (ImGui::TreeNode("Internal state"))
	{
		...
		ImGui::Text("DragDrop: %d, SourceId = 0x%08X, Payload \"%s\" (%d bytes)", g.DragDropActive, g.DragDropPayload.SourceId, g.DragDropPayload.NetworkTag, g.DragDropPayload.DataSize);
		ImGui::Unindent();
