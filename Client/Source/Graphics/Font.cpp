...
Font::Font(const char* filename, int maxSpriteCount)
{
	...
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\FontVS.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, "Shader\\FontPS.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
	...
	// .fnt 読み込み
	{
		...
		char* ret = nullptr;
		// face
		ret = ::strtok_s(nullptr, delimiter, &context);
...
DirectX::XMFLOAT2 Font::CalcStringSize(const wchar_t* string, float xScale, float yScale)
{
	...
	float width = x, height = fontHeight * yScale;
	float space = fontWidth * xScale;
	for (size_t i = 0; i < length; ++i)
	{
		// 文字値から文字情報が格納されているコードを取得
